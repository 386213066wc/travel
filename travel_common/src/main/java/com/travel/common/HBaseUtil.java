...
public class HBaseUtil {
    ...
    public static void createTable(Connection connection, String tableNameString, String columnFamily) throws IOException {
        ...
        TableDescriptorBuilder tableDescriptorBuilder = TableDescriptorBuilder.newBuilder(TableName.valueOf(tableNameString));
...
//        table.addFamily(family);
        ...
        if (!admin.tableExists(tableName)) {
            ...
            TableDescriptor descriptor = table1.getDescriptor();
...
//            HColumnDescriptor[] columnFamilies = tableDescriptor.getColumnFamilies();
            for (ColumnFamilyDescriptor family : columnFamilies) {
                String nameAsString = family.getNameAsString();
                if(!columnFamily.equals(nameAsString)){
                    ColumnFamilyDescriptor myColumnFamilyDescriptor = ColumnFamilyDescriptorBuilder.newBuilder(columnFamily.getBytes()).build();
                    admin.modifyColumnFamily(TableName.valueOf(tableNameString.getBytes()),myColumnFamilyDescriptor);
                }
            }
        }
        admin.close();
